...
int main() {
    const char connect[] = "ESP:connect\n"; //Connect String for ESP
    const char url[] = "ESP:url: \"http://192.168.1.250:5000/data\"\n"; //Command for ESP with the url of server
    const char transmit[] = "ESP:transmit\n"; //Transmit command for ESP
    char answer[8];
    ...
    print_lcd("2.Success");
    // Variables below control lcd shift.Go back after all characters are shown.
    unsigned int delay_cyc = 20;
    unsigned int go_back = 7;
    while(1){
        ...
        if(current_status != IDLE){
            ...
            snprintf(payload, 1024,
                 ...
                 "{\"name\": \"team\",\"value\": \"53\"},"
                 "{\"name\": \"status\", \"value\": \"%s\"}]\n",
                 temp, pressure,text_status[current_status]);
            send_string(payload);
            get_ans(answer);
            lcd_clear_display();
